...
#Testar klassen Hashtabell i filen Hashtabell.py
...
def lagraHashtabell(atomlista):
   ...
   # print(" * Lagrar listans atomer i hashtabell...")
    filstorlek = len(atomlista)
    hashtabell = Hashtabell(filstorlek)
    for element in atomlista:
        namn, vikt = element.split()
        ny = Atom(namn, float(vikt))
        hashtabell.store(namn, ny)
    return hashtabell
...
def allaAtomerFinns(hashtabell, atomlista):
    ...
    for kontrollAtom in atomlista:
        ...
        try:
            ...
            if hashadAtom.nodvalue.vikt != vikt:
